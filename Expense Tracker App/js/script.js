$(document).ready(function () {
  ...
  // Summary calculation
   ...
   // Render Pie Chart for category distribution
    ...
    // Initialize the data for categories (set default value to 0)
    ...
    // Ensure each category gets its respective color, even if it has no expense (0)
    ...
    const data = {
      labels: allCategories,  // Show all categories as labels
      datasets: [{
        data: dataValues,  // Data values for each category
        backgroundColor: backgroundColors,  // Set the background color for each category
      }],
    };
...
const displayRecentTransactions = () => {
  ...
  $(".nav-btn").click(function () {
      ...
      // Recalculate the summary if navigating to Summary page
      if (page === "summary") {
          calculateSummary();
      }     
  });
